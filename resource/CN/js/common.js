...
var bodyFixedClose = function(){
    offsetY = window.pageYOffset;  // 23.01.17 추가
    $("body").css({
        position: "",
        width: "",
        height: "",
    ...
    */
    ...
    if(windowWidth > 1280){
        ...
        $("#gnb .nv_btn").click(function(e){  // 23.02.08 수정
            bodyFixed()
            searchHide()
            $(".utility").css({top:windowHeight-136+"px"})
            e.stopImmediatePropagation();
        });
        ...
        $(".gnb_inner").css({top:headerHeight+"px", height:(windowHeight-197)+"px"})
...
$(function() {
    ...
    $(window).scroll(function(){
        ...
        if($(window).width() < 768){
            if(windowTop > 100 && windowTop < val + 200){
                $(".to_top, .special_menu").fadeIn();
                $(".to_top").css({bottom:""})
                $(".special_menu").css({bottom:""})
            }else{
                $(".to_top").fadeOut();
            }
            if (windowTop >= val && windowTop < val + 200){
                $(".to_top").css({bottom:(footerHeight / 5) * 2});
                $(".special_menu").css({bottom:(footerHeight / 5) * 2 + 58})
            }
            if (windowTop >= val + 200){
                $(".to_top, .special_menu").fadeOut();            
            }
        } else {
            if(windowTop > 100){
                $(".to_top").fadeIn();
            ...
            if (windowTop >= val){
                ...
                $(".special_menu").addClass("btm");
            }
            else{
                
                $(".special_menu").removeClass("btm");
